package org.dows.sdk.weixin.pay.基础支付.Native支付;
 ...
 */
public interface Native支付Api{

    /**
     * description: 统一下单API,最新更新时间：2020.05.26,商户Native支付统一下单接口，微信后台系统返回链接参数code_url，商户后台系统将code_url值生成二维码图片，用户使用微信客户端扫码后发起支付。
     ...
     * 
     * @param native下单Request
     * 
     */
    Native下单Response native下单(Native下单Request native下单Request);

    /**
     * description: 查询订单API,最新更新时间：2021.1.14,商户可以通过查询订单接口主动查询订单状态，完成下一步的业务逻辑。查询订单状态可通过微信支付订单号或商户订单号两种方式查询
    ...
    /**
     * description: 关单API,最新更新时间：2020.05.26,以下情况需要调用关单接口：,1、商户订单支付失败需要生成新单号重新发起支付，要对原订单号调用关单，避免重复支付；,2、系统下单后，用户支付超时，系统退出不再受理，避免用户继续，请调用关单接口。
    ...
    /**
     * description: 支付通知API,最新更新时间：2022.03.04,微信支付通过支付通知接口将用户支付成功消息通知给商户
