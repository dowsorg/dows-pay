package org.dows.sdk.weixin.thirdparty.management.request;

import io.swagger.v3.oas.annotations.media.Schema;
import lombok.Data;

/**
 * https://developers.weixin.qq.com/doc/oplatform/openApi/OpenApiDoc/thirdparty-management/domain-mgnt/getThirdpartyJumpDomainConfirmFile.html
 *
 * @author lait.zhang@gmail.com
 * @description
 * @date 2023年5月29日 上午11:32:00
 */
@Data
@Schema(name = "GetThirdpartyJumpDomainConfirmFileRequest", title = "GetThirdpartyJumpDomainConfirmFileRequest")
public class GetThirdpartyJumpDomainConfirmFileRequest {
}

