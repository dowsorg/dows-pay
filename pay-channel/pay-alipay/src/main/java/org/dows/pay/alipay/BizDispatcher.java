...
import org.dows.pay.alipay.constants.AlipayServiceEventConstants;
import org.dows.pay.alipay.constants.AlipayServiceNameConstants;
import org.dows.pay.alipay.executor.*;
...
public class BizDispatcher {
     ...
     * @throws PayException
     */
    public static ActionExecutor getExecutor(Map<String, String> params) throws PayException {
        //获取服务信息
        String service = params.get("service");
        if (StringUtils.isEmpty(service)) {
            throw new PayException("无法取得服务名");
        }
        //获取内容信息
        String bizContent = params.get("biz_content");
        if (StringUtils.isEmpty(bizContent)) {
            throw new PayException("无法取得业务内容信息");
        }
        //将XML转化成json对象
        JSONObject bizContentJson = JSONUtil.parseFromXml(bizContent).getJSONObject("XML");

        // 1.获取消息类型信息 
        String msgType = bizContentJson.getStr("MsgType");
        if (StringUtils.isEmpty(msgType)) {
            throw new PayException("无法取得消息类型");
        }
    ...
    private static ActionExecutor getEventExecutor(String service, JSONObject bizContentJson)
            throws PayException {
        // 1. 获取事件类型
        String eventType = bizContentJson.getStr("EventType");

        if (StringUtils.isEmpty(eventType)) {
            throw new PayException("无法取得事件类型");
        }
    ...
    private static ActionExecutor getEnterEventTypeExecutor(JSONObject bizContentJson) {
        try {
            ...
            JSONObject param = new JSONObject(bizContentJson.get("ActionParam"));
            JSONObject scene = new JSONObject(param.get("scene"));

            if (!StringUtils.isEmpty(scene.getStr("sceneId"))) {
                //自定义场景参数进入服务窗事件
    ...
    private static ActionExecutor getClickEventExecutor(JSONObject bizContentJson) {
        ...
        String actionParam = bizContentJson.getStr("ActionParam");
